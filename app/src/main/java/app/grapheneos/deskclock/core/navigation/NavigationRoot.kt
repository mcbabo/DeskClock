        ...
        entryProvider = entryProvider {
            ...
            entry<Route.Settings> {
                SettingsScreen(
                    ...
                    onBack = { backStack.removeLastOrNull() },
                    onNavigateToAlarmStylePicker = { backStack.add(Route.AlarmStylePicker) },
                    onNavigateToTimerStylePicker = { backStack.add(Route.TimerStylePicker) }
                )
            }

            entry<Route.AlarmStylePicker> {
                AlarmStylePickerScreen(
                    state = settingsUiState,
                    onIntent = settingsViewModel::handleIntent,
                    onBack = { backStack.removeLastOrNull() }
                )
